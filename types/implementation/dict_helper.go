...
func (dh DictHelper) Keys(dict baseIDict) baseIList {
	...
	result := dh.CreateList(0, dict.Len())

	for i := range keys {
		result.Append(keys[i])
	}
	return dh.ConvertList(result)
}
...
// Register the implementation of dictionary functions
var _ = func() int {
	types.AsDictionary = baseDictHelper.AsDictionary
	types.CreateDictionary = baseDictHelper.CreateDictionary
	types.TryAsDictionary = baseDictHelper.TryAsDictionary
	return 0
}()
