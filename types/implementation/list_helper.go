...
func (lh ListHelper) Append(list baseIList, objects ...interface{}) baseIList {
	target := list.AsArray()
	for i := range objects {
		*target = append(*target, lh.Convert(objects[i]))
	}
	return lh.ConvertList(list)
}
...
func (lh ListHelper) SetIndex(list baseIList, index int, value interface{}) (baseIList, error) {
	...
	return lh.ConvertList(list), nil
}

// Register the implementation of list functions
var _ = func() int {
	types.CreateList = baseListHelper.CreateList
	types.NewList = baseListHelper.NewList
	types.NewStringList = baseListHelper.NewStringList
	types.TryAsList = baseListHelper.TryAsList
	return 0
}()
